...
def create_sat_vis_heatmap(obj_id, js_id, js_format, selected_vars):
    ...
    with open(text_file.content.path) as f:
        ...
        for index, line in enumerate(text):
            ...
            if first_letter in ['c', 'C', 'p'] or line in ['', ' ']:
                # If line is a comment:
                if first_letter in ['c', 'C'] or line in ['', ' ']:
                    index_offset += 1
                    continue

                # If line is a cnf specification:
                if first_letter == 'p':
                    index_offset += 1
                ...
                line_variables_list = [int(i) for i in line[:-2].split()[:] if i != '']
                for variable in line_variables_list:
                    if variable > 0:
                        sat[index-index_offset][variable-1] = 1
                    else:
                        sat[index-index_offset][-variable-1] = -1
                # Number of variables in a clause

                cl_dict[index-index_offset] = np.count_nonzero(sat[index-index_offset])

    print("Formatting cnf DONE")
    ...
    for i in range(len(heatmap)):
        ...
        for j in range(len(heatmap[i])):
            val_ctr += 1
            heatmap[i][j] = (val_dict.get(val_ctr)/cl_dict.get(i))
    print("Full Resolution Heatmap Creation DONE")

    maximum_resolution = 1000
    #
    # while len(heatmap) >= maximum_resolution or len(heatmap[0]) >= maximum_resolution:
    #     heatmap = heatmap[::2, ::2]
    ...
    for row_idx, row in enumerate(heatmap):
        update_progress(2*lines_amount//3 + i//3, lines_amount, obj)
